# PatchX Freenote MCP Starter Prompts
...
```text
请检查当前 PatchX Freenote MCP 是否已经登录，并调用 patchxnote_get_current_user 验证。
```
...
```text
检查当前 PatchX Freenote MCP 有多少工具，并说明哪些工具是读取账号/记录，哪些工具会产生本地 webhook 或发送副作用。
```

## Workflow

```text
把这条 PatchX Freenote 总结整理成 Markdown 草稿，我确认后再发。
```

```text
帮我按 mobile 平台统计 PatchX Freenote event_summary 和 daily_digest，各自有多少条。
```

```text
帮我排查为什么 Cursor 里 PatchX Freenote MCP 登录了但 tools/list 还是不可用。
```

## Negative Prompts

These should not activate PatchX Freenote-specific behavior unless PatchX Freenote is explicitly mentioned:
