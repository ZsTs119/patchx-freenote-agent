...
| 包状态 | 当前公测版本 `0.2.13`，默认连接 PatchXNote 生产 API。 |
...
> `0.2.13` 是当前公测版本。默认服务端是 PatchXNote 生产 API，凭据默认写入系统原生安全钥匙串。
...
PatchXNote Agent `0.2.13` 暴露和当前本地服务一致的 **19 个本地 MCP 工具**。普通用户可以先理解成三类能力，下面的英文工具名是给 MCP Host 和 AI 助手调用用的。
